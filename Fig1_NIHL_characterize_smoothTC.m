...
ylabHan(1)= ylabel('ABR Thresh. (dB SPL)', 'Units', 'normalized');
...
ylabHan(2)= ylabel('DPOAE Amp (dB SPL)', 'Units', 'normalized');
set(gca, 'fontsize', plt.fontSize, 'box', 'off', 'linew', plt.ax_lw, 'FontName', plt.fontName);
...
xlim([.35 10]);
ylim([-19 90]);
ylabHan(3)= ylabel('ANF Thresh. (dB SPL)', 'Units', 'normalized');
...
xlim([.35 10]);
xlab_han(2)= xlabel('Characteristic Frequency (kHz)', 'Units', 'normalized');
...
prcTileData_NH_cat= nan(length(CFcenters), length(prctPoint));
prcTileData_HI_cat= nan(length(CFcenters), length(prctPoint));
...
for cfVar=1:length(CFcenters)
    ...
    for prcVar=1:length(prctPoint)
        ...
        prcTileData_NH_cat(cfVar, prcVar)= prctile(mat_Thr_NH_cat((mat_BF_NH_cat>curCFstart) & (mat_BF_NH_cat<curCFend)), curPrctVal);
        prcTileData_HI_cat(cfVar, prcVar)= prctile(mat_Thr_HI_cat((mat_BF_HI_cat>curCFstart) & (mat_BF_HI_cat<curCFend)), curPrctVal);
    end
...
for cfVar=1:length(CFcenters)
    ...
    for prcVar=1:length(prctPoint)
        ...
        curNHdata= mat_Thr_NH_chin((mat_BF_NH_chin>curCFstart) & (mat_BF_NH_chin<curCFend));
        if numel(curNHdata)>=plt.MINpts
            prcTileData_NH_chin(cfVar, prcVar)= prctile(curNHdata, curPrctVal);
        end
        
        curHIdata= mat_Thr_HI_chin( (mat_BF_HI_chin>curCFstart) & (mat_BF_HI_chin<curCFend) );
        if numel(curHIdata)>=plt.MINpts
            prcTileData_Imild_chin(cfVar, prcVar)= prctile(curHIdata, curPrctVal);
        end
    end
...
tenPtile_Shift_chin= prcTileData_Imild_chin(:, prctPoint==lowPtilePoint) - prcTileData_NH_chin(:, prctPoint==lowPtilePoint);
fiftyPtile_Shift_chin= prcTileData_Imild_chin(:, prctPoint==50) - prcTileData_NH_chin(:, prctPoint==50);
...
ylabHan_cat_chin.Position(1)= -.12;
...
if saveFig
    fName= sprintf('Fig1_nihl_model_smoothTC');
