...
if ~isempty(curData.pos)
    ...
    for winVar= 1:length(tWindows)
        ...
        for segVar= 1:numSegments
            ...
            NZinds_pos= nSpikes_pos~=0;
            NZpos= sum(nSpikes_pos==0);
            
            nSpikes_neg= cellfun(@(x) numel(x), cur_spikes_neg);
            NZinds_neg= nSpikes_neg~=0;
            NZneg= sum(nSpikes_neg==0);
            
            DistMin_pos= abs(nSpikes_pos(NZinds_pos) - nSpikes_pos(NZinds_pos)');
            DistMax_pos= abs(nSpikes_pos(NZinds_pos) + nSpikes_pos(NZinds_pos)');
            
            DistMin_neg= abs(nSpikes_neg(NZinds_neg) - nSpikes_neg(NZinds_neg)');
            DistMax_neg= abs(nSpikes_neg(NZinds_neg) + nSpikes_neg(NZinds_neg)');
            ...
            for costVar=1:length(anl.VPcosts)
                ...
                tempDist_2d_pos= (tempDist_2d_pos - DistMin_pos) ./ (DistMax_pos - DistMin_pos);
                
                tempDist_2d_neg= helper.VPspkdlPW(cur_spikes_neg(nSpikes_neg~=0), cur_spikes_neg(nSpikes_neg~=0), curCost);
                tempDist_2d_neg= (tempDist_2d_neg - DistMin_neg) ./ (DistMax_neg - DistMin_neg);
                
                up_tri_Mask_pos= triu(ones(size(tempDist_2d_pos)), 1)==1;
                dist_pos= [tempDist_2d_pos(up_tri_Mask_pos); repmat(zeros(size(nSpikes_pos(nSpikes_pos~=0))), NZpos, 1); zeros(NZpos*(NZpos-1)/2, 1)];
                
                up_tri_Mask_neg= triu(ones(size(tempDist_2d_neg)), 1)==1;
                dist_neg= [tempDist_2d_neg(up_tri_Mask_neg); repmat(zeros(size(nSpikes_neg(nSpikes_neg~=0))), NZneg, 1); zeros(NZneg*(NZneg-1)/2, 1)];
