...
set(gca, 'XScale', 'log', 'XTick', plt.freqTick, 'XTickLabel', '');
ylim(sp_ax(1), [0 171]);
...
[legHanA, icons]= leg_helper.columnlegend(lHan, length(lHan), legend_str, 'Location', 'NorthEast', 'boxoff');
legHanA.Position(1:2)= [.19 .865];
...
[legHanB, icons]= legend(lHan(1:2), 'NH', 'HI', 'Location', 'northeast', 'box', 'off');
count= 3;
...
xlabHanD= xlabel('Characteristic frequency (kHz)', 'Units', 'normalized');
xlabHanD.Position(2)= -.18;
...
drawnow
% A
set(sp_ax(1),'Position',[Xcorner_X, Ycorner_X+3*Ywidth_X+3*Yshift_X, Xwidth_X, 1.1*Ywidth_X])
drawnow

if saveFig
    print([dirStruct.png 'Fig4_dt_CFvsLF'], '-dpng',  '-r600');
    saveas(gcf, [dirStruct.eps 'Fig4_dt_CFvsLF'], 'epsc');
end
