...
anl_d.resp_mask= (anl.tBinCenters> anl_d.tStart) & (anl.tBinCenters<anl_d.tEnd); 
anl_g.resp_mask= (anl.tBinCenters> anl_g.tStart) & (anl.tBinCenters<anl_g.tEnd); 
...
sp_ax(1)= subplot(3, 3, 1);
yyaxis right;
hold on;
[~,~,lHan_d]= helper.plot_dpss_psd(sig.data(sig.d_inds), sig.fs, 'nw', anl.nw, 'xunit', 'khz');
set(lHan_d, 'color', helper.get_color('k'), 'linew', plt.lw1);
[~,~,lHan_g]= helper.plot_dpss_psd(sig.data(sig.g_inds), sig.fs, 'nw', anl.nw, 'xunit', 'khz');
...
legHan_plosive.Position(1:2)= [.205 .87];
...
plot(sig.time(sig.d_inds_zoom)+plt.sigAdjust, -Ashift+sig.data(sig.d_inds_zoom), 'Color', helper.get_color('k'), 'linew', plt.lw1);
...
yyaxis right;
plot(anl.tBinCenters, anl_d.resp_mask, '-', 'Color', helper.get_color('g'), 'linew', plt.lw1);
% plot(anl.tBinCenters, anl_d.SustainedMask, '-', 'Color', helper.get_color('m'), 'linew', plt.lw1);
ylim([-0.25 5]);
...
plot(anl.tBinCenters, demo.hi_uRate_pos, '-', 'Color', helper.get_color('r'), 'linew', plt.lw1);
plot(anl.tBinCenters, -demo.hi_uRate_neg, '-', 'Color', helper.get_color('lr'), 'linew', plt.lw1);
plot(sig.time(sig.g_inds_zoom)+plt.sigAdjust, -Ashift+sig.data(sig.g_inds_zoom), '-', 'Color', helper.get_color('gray'), 'linew', plt.lw1);
set(gca, 'xtick', plt.g_time_ticks);
set(gca, 'YTick', ytickVals, 'YTickLabel', ytickLabs, 'YColor', 'k');

yyaxis right;
plot(anl.tBinCenters, anl_g.resp_mask, '-', 'Color', helper.get_color('g'), 'linew', plt.lw1);
% plot(anl.tBinCenters, anl_g.SustainedMask, '-', 'Color', helper.get_color('m'), 'linew', plt.lw1);
ylim([-0.25 5]);
set(gca, 'YTick', [0 1], 'YColor', 'k');
box off;
...
Rates_d= nan(length(all_chinDanishData), 1);
Rates_g= nan(length(all_chinDanishData), 1);
...
parfor unitVar= 1:length(all_chinDanishData)
    ...
    Rates_d(unitVar)= ( sum(uR_pos.*anl_d.resp_mask) + sum(uR_neg.*anl_d.resp_mask) ) / 2 / (anl_d.tEnd - anl_d.tStart);
    Rates_g(unitVar)= ( sum(uR_pos.*anl_g.resp_mask) + sum(uR_neg.*anl_g.resp_mask) ) / 2 / (anl_g.tEnd - anl_g.tStart);
...
% /d/ Rate
sp_ax(4)= subplot(3, 2, 3);

hold on;
plot(all_cf_kHz(nhInds), Rates_d(nhInds), 'x', 'Color', helper.get_color('b'), 'MarkerSize', plt.mrkSize);
[~, ~, lHan] = helper.octAVG(all_cf_kHz(nhInds), Rates_d(nhInds), plt.MINpts, plt.octRange4Avging, plt.plotVar);
...
plot(all_cf_kHz(hiInds), Rates_d(hiInds), '+', 'Color', helper.get_color('r'), 'MarkerSize', plt.mrkSize);
[~, ~, lHan] = helper.octAVG(all_cf_kHz(hiInds), Rates_d(hiInds), plt.MINpts, plt.octRange4Avging, plt.plotVar);
...
xlab_cf_han.Position(2)= -.13;

% /g/ Rate
sp_ax(5)= subplot(3, 2, 4);

hold on;

plot(all_cf_kHz(nhInds), Rates_g(nhInds), 'x', 'Color', helper.get_color('b'), 'MarkerSize', plt.mrkSize);
[~, ~, lHan] = helper.octAVG(all_cf_kHz(nhInds), Rates_g(nhInds), plt.MINpts, plt.octRange4Avging, plt.plotVar);
set(lHan, 'LineStyle', '-', 'LineWidth', plt.lw3, 'color', 'b');
...
plot(all_cf_kHz(hiInds), Rates_g(hiInds), '+', 'Color', helper.get_color('r'), 'MarkerSize', plt.mrkSize);
[~, ~, lHan] = helper.octAVG(all_cf_kHz(hiInds), Rates_g(hiInds), plt.MINpts, plt.octRange4Avging, plt.plotVar);
set(lHan, 'LineStyle', '-', 'LineWidth', plt.lw3, 'color', 'r');
...
% H
set(sp_ax(6),'Position', [Xcorner Ycorner Xwidth .75*Ywidth])
drawnow
...
% I
set(sp_ax(7),'Position', [Xcorner+Xwidth+Xshift Ycorner Xwidth .75*Ywidth])
drawnow

% G
set(sp_ax(5),'Position', [Xcorner+Xwidth+Xshift Ycorner+Ywidth+Yshift Xwidth Ywidth])
drawnow

yCornerTop= Ycorner+2*Ywidth+4*Yshift;

% A
set(sp_ax(1),'Position', [Xcorner yCornerTop Xwidth_top Ywidth])
drawnow

% B
set(sp_ax(2),'Position', [Xcorner+Xwidth_top+1.65*Xshift_top yCornerTop .95*Xwidth_top Ywidth])
drawnow

% C
set(sp_ax(3),'Position', [Xcorner+1.9*Xwidth_top+2.5*Xshift_top yCornerTop .95*Xwidth_top Ywidth])
drawnow


if saveFig
    print([dirStruct.png 'Fig7_plosive_burst'], '-dpng',  '-r600');
end
...
if saveStats
    writetable(Fig7_tbl_plosive_coding_FFR, [dirStruct.stats 'Fig7_FFR_plosive_all_burst.txt'], 'Delimiter', 'tab');
end