...
demo.nh_uRate_pos= histcounts( cell2mat(demo.nh_data.pos) - demo.delay, anl.tBinEdges) / length(demo.nh_data.pos) * fs;
demo.nh_uRate_neg= histcounts( cell2mat(demo.nh_data.neg) - demo.delay, anl.tBinEdges) / length(demo.nh_data.neg) * fs;
demo.hi_uRate_pos= histcounts( cell2mat(demo.hi_data.pos) - demo.delay, anl.tBinEdges) / length(demo.hi_data.pos) * fs;
demo.hi_uRate_neg= histcounts( cell2mat(demo.hi_data.neg) - demo.delay, anl.tBinEdges) / length(demo.hi_data.neg) * fs;
...
xlabel('');
yLabhHan_Ar= ylabel('PSD (dB/Hz)', 'Units', 'normalized');
yLabhHan_Ar.Position(1)= 1.125;
...
yLabhHan_Bl= ylabel('Rate (spikes/s)', 'Units', 'normalized');
yLabhHan_Bl.Position(1)= -.13;
...
for dirVar= 1:length(allDirs)
    ...
    if ~isempty(s_files)
        ...
        if s_atten<15
            ...
            t_ffr_valid= (t_data>min(plt.FFR_tLim)) & (t_data<max(plt.FFR_tLim));
            ...
            if ChinID==ffr_demo.nh_ffr_chin
                ...
                yyaxis right;
                hold on;
                plot(t_data(t_ffr_valid), ffr_OnsetMask(t_ffr_valid), '-', 'color', helper.get_color('c'), 'linew', plt.lw2)
                ylim([-.5 5]);
                xlim(plt.FFR_tLim);
...
if saveFig
    ...
    saveas(gcf, [dirStruct.eps 'Fig6_fric_s_rel2SR'], 'epsc');
end
