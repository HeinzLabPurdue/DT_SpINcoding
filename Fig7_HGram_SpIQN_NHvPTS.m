...
if use_trifilt
    danish.trajectory.f0= helper.trifilt(danish.trajectory.f0, anl.trifilt_nPoints);
    danish.trajectory.f1= helper.trifilt(danish.trajectory.f1, anl.trifilt_nPoints);
    danish.trajectory.f2= helper.trifilt(danish.trajectory.f2, anl.trifilt_nPoints);
    danish.trajectory.f3= helper.trifilt(danish.trajectory.f3, anl.trifilt_nPoints);
end
...
plt.MINpts= 1;
plt.lw= 1.5;
plt.lw_Fx= 1.25; % Linewidth for formant trajectory 
plt.xtick_freq_val= [200 500 1e3 2e3 5e3];
...
for snrVar= 1:length(allSNRs)
    ...
    [~, ~, lHan] = helper.octAVG(all_cf_Hz(nhInds), F1_strength_rel(nhInds), plt.MINpts, plt.octRange4Avging, plt.plotVar, [], stat_params.cf_min*1e3);
    ...
    [~, ~, lHan] = helper.octAVG(all_cf_Hz(hiInds), F1_strength_rel(hiInds), plt.MINpts, plt.octRange4Avging, plt.plotVar, [], stat_params.cf_min*1e3);
    set(lHan, 'LineStyle', '-', 'LineWidth', plt.lw, 'color', 'r');
    set(gca, 'YColor', 'k', 'TickLength', plt.tick_len);
    ylim([0 0.65])
    
    yyaxis right;
    plot(danish.trajectory.f1, anl.tBinCenter, 'k', 'linew', plt.lw_Fx);
    if snrVar==length(allSNRs)
        set(gca, 'XScale', 'log', 'YColor', 'k', 'xtick', plt.xtick_freq_val, 'XTickLabel', plt.xtick_freq_lab, 'YTickLabel', '');
    else 
        set(gca, 'XScale', 'log', 'YColor', 'k', 'xtick', plt.xtick_freq_val, 'XTickLabel', '', 'YTickLabel', '');
    end
    ...
    count= count+1;
    txtHan(count)= text(.15, 1.05, sprintf('%s', spLetters(count)), 'Units', 'normalized', 'FontWeight', 'bold');
    ...
    [~, ~, lHan] = helper.octAVG(all_cf_Hz(nhInds), F2_strength_rel(nhInds), plt.MINpts, plt.octRange4Avging, plt.plotVar, [], stat_params.cf_min*1e3);
    set(lHan, 'LineStyle', '-', 'LineWidth', plt.lw, 'color', 'b');
    ...
    [~, ~, lHan] = helper.octAVG(all_cf_Hz(hiInds), F2_strength_rel(hiInds), plt.MINpts, plt.octRange4Avging, plt.plotVar, [], stat_params.cf_min*1e3);
    set(lHan, 'LineStyle', '-', 'LineWidth', plt.lw, 'color', 'r');
    set(gca, 'YColor', 'k', 'TickLength', plt.tick_len);
    ylim([0 0.175])
    
    yyaxis right;
    plot(danish.trajectory.f2, anl.tBinCenter, 'k', 'linew', plt.lw_Fx);
    ...
    F3_strength_rel= F3_strength_rel*1e2;
    ...
    [~, ~, lHan] = helper.octAVG(all_cf_Hz(nhInds), F3_strength_rel(nhInds), plt.MINpts, plt.octRange4Avging, plt.plotVar, [], stat_params.cf_min*1e3);
    set(lHan, 'LineStyle', '-', 'LineWidth', plt.lw, 'color', 'b');
    text(-.23, 1.05, 'x10^2', 'Units', 'normalized');
    ...
    [~, ~, lHan] = helper.octAVG(all_cf_Hz(hiInds), F3_strength_rel(hiInds), plt.MINpts, plt.octRange4Avging, plt.plotVar, [], stat_params.cf_min*1e3);
    set(lHan, 'LineStyle', '-', 'LineWidth', plt.lw, 'color', 'r');
    set(gca, 'YColor', 'k', 'TickLength', plt.tick_len);
    ...
    yyaxis right;
    plot(danish.trajectory.f3, anl.tBinCenter, 'k', 'linew', plt.lw_Fx);

    if snrVar==length(allSNRs)
        set(gca, 'XScale', 'log', 'YColor', 'k', 'xtick', plt.xtick_freq_val, 'XTickLabel', plt.xtick_freq_lab);
    else
        set(gca, 'XScale', 'log', 'YColor', 'k', 'xtick', plt.xtick_freq_val, 'XTickLabel', '');
    end
...
ttlHan(1)= text(-.45, .25, 'Quiet', 'Units', 'normalized', 'FontWeight', 'bold', 'VerticalAlignment', 'middle', 'Rotation', 90);
subplot(length(allSNRs), 3, 4);
ttlHan(2)= text(-.45, 0.09, '0 dB SNR', 'Units', 'normalized', 'FontWeight', 'bold', 'VerticalAlignment', 'middle', 'Rotation', 90);
subplot(length(allSNRs), 3, 7);
ttlHan(3)= text(-.45, .08, '-5 dB SNR', 'Units', 'normalized', 'FontWeight', 'bold', 'VerticalAlignment', 'middle', 'Rotation', 90);
...
axes(sp_ax_X(4));
yyaxis left
yLab_fPow_Han= ylabel('Formant power', 'Units', 'normalized');
yLab_fPow_Han.Position(1)= -.24;
...
axes(sp_ax_X(end-1));
xLabHan= xlabel('Characteristic frequency or frequency (kHz)', 'Units', 'normalized');
xLabHan.Position(2)= -.32;
...
if (use_FracSig0_FracHGram1_SynInd2_FracLPpow3==1) && saveFig
    ...
    saveas(gcf, [dirStruct.eps 'Fig5_HGram_SpIQN_NHvPTS'], 'epsc');
end